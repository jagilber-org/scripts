...
# Load SF module if cmdlets not available
`$_sfModulePath = 'C:\Program Files\Microsoft Service Fabric\bin\Fabric\Fabric.Code\ServiceFabric.psd1'
if (-not (Get-Command 'Get-ServiceFabricNode' -ErrorAction SilentlyContinue)) {
...
if ((Get-Command 'Connect-ServiceFabricCluster' -ErrorAction SilentlyContinue) -and `$Phase -ne 'PreFlight') {
    if (`$ManagementEndpoint -match '^<.*>`$') {
...
switch (`$Phase) {
    ...
    'PostDeploy' {
        ...
        `$vmss = Get-AzVmss -ResourceGroupName `$ResourceGroupName -VMScaleSetName `$ReplacementVmssName -ErrorAction SilentlyContinue
        Assert-Check 'Replacement VMSS exists' (`$null -ne `$vmss) "VMSS `$ReplacementVmssName not found"
        if (`$vmss) {
    ...
    'PostDrain' {
        ...
        Assert-Check 'Replacement nodes are Up' (`$upNodes.Count -ge 5) "Only `$(`$upNodes.Count) Up nodes"
        Assert-Check 'Seeds on replacement VMSS' (`$seedNodes.Count -ge 3) "Only `$(`$seedNodes.Count) active seed nodes"
    ...
    'PostCleanup' {
        ...
        Assert-Check 'At least 5 healthy nodes' (`$allUp.Count -ge 5) "Only `$(`$allUp.Count) Up nodes"
