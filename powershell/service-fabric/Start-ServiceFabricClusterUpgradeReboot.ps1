...
.DESCRIPTION
    Triggers a Service Fabric cluster configuration upgrade with forceRestart enabled.
    This causes a fabrichost.exe restart across all upgrade domains for applying settings.

.NOTES

    File Name  : Start-ServiceFabricClusterUpgradeReboot.ps1
...
.EXAMPLE
    .\Start-ServiceFabricClusterUpgradeReboot.ps1 -resourceGroup 'myRG' -clusterName 'myCluster'
