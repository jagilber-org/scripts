<#
.SYNOPSIS
    Converts Service Fabric ETL trace files to CSV text format.

.DESCRIPTION
    Converts ETL trace files from a Service Fabric node to human-readable CSV format.
    Downloads the required manifest CAB files for trace parsing from the Service Fabric
    runtime distribution. Must be run from the cluster node.

.NOTES

    File Name  : Convert-ServiceFabricEtl.ps1
...
.EXAMPLE
    .\Convert-ServiceFabricEtl.ps1 -sfLogDir 'd:\svcfab\log' -outputDir 'd:\temp'
#>
