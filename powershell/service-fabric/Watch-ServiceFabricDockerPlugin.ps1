...
.DESCRIPTION
    Monitors the sfazurefile Docker volume plugin configuration files for changes.
    Reports file handles and content modifications at a configurable interval.

.NOTES

    File Name  : Watch-ServiceFabricDockerPlugin.ps1
...
.EXAMPLE
    .\Watch-ServiceFabricDockerPlugin.ps1 -sleepSeconds 10
...
param(
    ...
    $errorBehavior = 'silentlycontinue'
)
...
function main() {
    ...
    while ($true) {
        ...
        docker images;
        #docker stats;
        foreach ($f in (get-childitem $fileFilter -Recurse -ErrorAction $errorBehavior)) {
            if ($useHandle) {
