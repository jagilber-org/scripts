...
.DESCRIPTION
    Prunes Docker images, containers, volumes, or networks on a Service Fabric node.
    Supports filtering by image name and time-based cleanup.

.NOTES

    File Name  : Clear-ServiceFabricDocker.ps1
...
.EXAMPLE
    .\Clear-ServiceFabricDocker.ps1 -action 'image' -whatIf
...
function main() {
    ...
    foreach ($service in $servicesTable) {
        ...
        set-service -name $service.name -StartupType Disabled -WhatIf:$whatIf -ErrorAction $errorBehavior

        write-output "stopping $($service.name)"
        stop-service -name $service.name -WhatIf:$whatIf -ErrorAction $errorBehavior -Force:$force
    }
    ...
    foreach ($service in ($servicesTable | sort-object)) {
        write-output "setting $($service.name) to $($service.startuptype)"
        set-service -name $service.name -StartupType $service.startuptype -WhatIf:$whatIf -ErrorAction $errorBehavior

        if ($service.restart) {
            write-output "starting $($service.name)"
            start-service -name $service.name -WhatIf:$whatIf -ErrorAction $errorBehavior
        }
    }
