<#
.SYNOPSIS
    Queries the Service Fabric REST API on localhost using a client certificate.

.DESCRIPTION
    Sends REST requests to the Service Fabric cluster management endpoint.
    Uses client certificate authentication for secure clusters.
    Supports querying nodes, applications, services, health, and other cluster resources.

.NOTES

    File Name  : Invoke-ServiceFabricRestApi.ps1
...
.EXAMPLE
    .\Invoke-ServiceFabricRestApi.ps1 -gatewayHostUrl 'https://localhost:19080' -gatewayCertThumb '<thumbprint>'
