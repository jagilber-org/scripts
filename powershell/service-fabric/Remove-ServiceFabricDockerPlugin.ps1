<#
.SYNOPSIS
    Removes sfazurefile.json from Docker plugin directory on a Service Fabric node.

.DESCRIPTION
    Removes or renames the sfazurefile Docker volume plugin configuration file.
    Can optionally stop related services and processes before removal.

.NOTES

    File Name  : Remove-ServiceFabricDockerPlugin.ps1
...
.EXAMPLE
    .\Remove-ServiceFabricDockerPlugin.ps1 -whatIf -stopServices
...
function main() {
    ...
    $pluginFiles = get-childitem -Path $dockerRoot -Filter $pluginFile -Recurse -ErrorAction $errorBehavior
    ...
    if ($stopServices) {
        foreach ($service in $services) {
            ...
            stop-service -name $service -WhatIf:$whatIf -ErrorAction $errorBehavior -Force:$force
        }
    }
    ...
    if ($stopProcesses) {
        foreach ($process in $processes) {
            ...
            stop-process -name $process -WhatIf:$whatIf -ErrorAction $errorBehavior -Force:$force
        }
    }
    ...
    foreach ($f in $pluginFiles) {
        ...
        if (!$isLocked) {
            if ($action -ieq 'remove') {
                write-output "$(get-date):remove-item $fileName -WhatIf:$whatIf -ErrorAction $errorBehavior -Force:$force"
                remove-item $fileName -WhatIf:$whatIf -ErrorAction $errorBehavior -Force:$force
            }
            elseif ($action -ieq 'rename') {
                write-output "$(get-date):rename-item -path $fileName -NewName `"$fileName.old`" -WhatIf:$whatIf -ErrorAction $errorBehavior -Force:$force"
                rename-item -path $fileName -NewName "$fileName.old" -WhatIf:$whatIf -ErrorAction $errorBehavior -Force:$force
            }
            else {
    ...
    if ($stopServices) {
        foreach ($service in ($services | sort-object)) {
            ...
            start-service -name $service -WhatIf:$whatIf -ErrorAction $errorBehavior
        }
    }
