<#
.SYNOPSIS
    Creates or manages Windows scheduled tasks with support for script file downloads, multiple trigger frequencies, and configurable authentication principals.

.DESCRIPTION
    Creates a Windows scheduled task that can execute PowerShell scripts or commands on a configurable
    schedule. Supports downloading script files from URLs, multiple trigger frequencies (startup, once,
    daily, weekly), and configurable authentication principals. Writes event 4103 to
    Microsoft-Windows-PowerShell Operational event log on completion.

.NOTES

    File Name  : New-ScheduledTask.ps1
