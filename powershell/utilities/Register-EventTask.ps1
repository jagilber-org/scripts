...
Author    : jagilber
 Version    : 1.0.0
 Changelog  : 1.0.0 - Version normalization from 141206 (constitution quality-007/008/009)
.EXAMPLE
...
param(
    ...
    [bool] $install = $false,
    [parameter(Position=1,Mandatory=$false,HelpMessage="Enter `$true to uninstall event log monitor")]
    [bool] $uninstall = $false,
    [parameter(Position=2,Mandatory=$false,HelpMessage="Enter `$true to test email")]
    [bool] $test = $false,
    [parameter(Position=3,Mandatory=$false,HelpMessage="Enter working directory")]
    [string] $workingDir
)
