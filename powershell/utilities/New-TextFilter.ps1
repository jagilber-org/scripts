<#
.SYNOPSIS
    Generates a TextFilter .rvf filter file from ETW trace metadata.

.DESCRIPTION
    Creates a TextFilter .rvf XML filter file from ETW inf.txt trace metadata files. Supports multiple input files.

.NOTES

    File Name  : New-TextFilter.ps1
...
.EXAMPLE
    .\New-TextFilter.ps1 -infFile 'C:\traces\inf.txt'
    Generates a .rvf filter file from the specified ETW metadata file.
#>
