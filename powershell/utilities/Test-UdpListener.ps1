<#
.SYNOPSIS
    UDP client/server test tool for connectivity troubleshooting.

.DESCRIPTION
    Starts a UDP listener or sends UDP packets as a client to troubleshoot network connectivity issues.

.NOTES

    File Name  : Test-UdpListener.ps1
...
.EXAMPLE
    .\Test-UdpListener.ps1 -server -port 514
    Starts a UDP listener on port 514.

.EXAMPLE
    .\Test-UdpListener.ps1 -remoteAddress 'server1' -port 514 -count 10
    Sends 10 UDP packets to server1 on port 514.
#>
