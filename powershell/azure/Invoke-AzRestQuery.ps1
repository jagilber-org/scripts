<#
.SYNOPSIS
    Invokes Azure Resource Manager REST API queries using a bearer token.
    Requires authentication via Connect-AzRestApi.ps1.

.DESCRIPTION
    Sends REST requests (GET, POST, PUT) to the Azure Resource Manager API.
    Uses the bearer token stored in $global:token by Connect-AzRestApi.ps1.
    Supports querying resources, resource groups, providers, and other ARM endpoints.

.NOTES

    File Name  : Invoke-AzRestQuery.ps1
...
.EXAMPLE
    .\Invoke-AzRestQuery.ps1 -query 'resources'
    Lists all resources in the current subscription.

.EXAMPLE
    .\Invoke-AzRestQuery.ps1 -query 'resourceGroups'
...
param(
    ...
    # Well-known Azure PowerShell first-party client ID (public, non-secret)
