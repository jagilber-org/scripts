<#
.SYNOPSIS
    Retrieves the OS image reference for VMSS instances in a Service Fabric cluster.

.DESCRIPTION
    Queries a Service Fabric cluster for its VMSS node types and returns the current
    OS image information (publisher, offer, SKU, version) for each scale set.

.NOTES

    File Name  : Get-AzVmssImage.ps1
...
.EXAMPLE
    .\Get-AzVmssImage.ps1 -resourceGroupName 'myRG' -nodeTypeName 'nt0'
